...
def build_gatekeeper_model():
    """Builds the binary Sigmoid EfficientNet-B3 for our ID Checker."""
    model = models.efficientnet_b3(weights=None)
    num_ftrs = model.classifier[1].in_features
    model.classifier[1] = nn.Linear(num_ftrs, 1) # Single output for Sigmoid
    return model
...
# Model 0: "Gatekeeper"
gatekeeper_model = build_gatekeeper_model()
gatekeeper_path = hf_hub_download(
    repo_id="Bhoomika18/cervical-cancer-models",
    filename="gatekeeper_model.pth" # Make sure you upload this file!
)
gatekeeper_model.load_state_dict(torch.load(gatekeeper_path, map_location=DEVICE))
gatekeeper_model.eval().to(DEVICE)
print("✅ 'Gatekeeper' model (ID Checker) loaded from Hugging Face.")
...
def run_gatekeeper_check(image):
    """Checks if an image is valid using the raw logit output. Returns True if valid, False otherwise."""
    if image is None:
        return False, None # Return None for the tensor if image is None
    input_tensor = preprocess(image.convert("RGB")).unsqueeze(0).to(DEVICE)
    with torch.no_grad():
        output = gatekeeper_model(input_tensor)
        # Use the original threshold from your friend's code, assuming the output is a raw logit
        is_valid = (output.item() > 2.5) # Reverted back to original logic
    return is_valid, input_tensor


# ✨ NEW: Helper function to save feedback to Supabase


# ✨ MODIFIED: Helper function to save feedback to Supabase
# NOTE: We added 'user_id' and 'user_role' to the inputs.
def save_feedback(session_id, stage, image, ai_prediction, feedback, correct_label, user_id, user_role):
    """Uploads image to storage and saves feedback to the database, including user details."""
    ...
    try:
        data = {
            ...
            # ✨ NEW LINES: Add user data for tracking
            "user_id": user_id,
            "user_role": user_role
        }
        supabase.table("feedback_log").insert(data).execute()
        
        success_msg = f"✅ Feedback for stage '{stage}' saved by user {user_id}."
        print(success_msg)
...
def run_surveyor_analysis(image):
    ...
    # ✨ ADD GATEKEEPER CHECK HERE ✨
    is_valid, _ = run_gatekeeper_check(image) # We don't need the tensor again here
    if not is_valid:
        report = "Sorry, I can't recognise the image. Please upload a valid colposcopy image."
        # Need to return enough None values to match the expected outputs
        return report, None, None, None, None, None, gr.update(visible=False), gr.update(visible=False)
...
def run_screener_analysis(image):
    ...
    # --- Gatekeeper Check ---
    is_valid, _ = run_gatekeeper_check(image)
    if not is_valid:
        report = "Sorry, I can't recognise the image. Please upload a valid diagnostic image."
        # Return report for screener_probs AND debug_output, plus Nones/updates for others
        return report, None, None, gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), report # Show error in both places
    # --- End of Gatekeeper Check ---
    ...
    return (
        ...
        screener_result,
        image,
        gr.update(visible=True), # Show screener feedback
        gr.update(visible=False), # Hide grader feedback
        gr.update(visible=False), # Hide RL group
        "" # Empty string for debug output on success
    )
...
# NOTE: In a real app, this would check a secure database. Here it's mocked.
def authenticate_user(username, password):
    """Mocks authentication and sets user role/ID."""
    if username == "admin" and password == "123":
        # This is for your demo/testing/presentation
        return "admin_bhoomika", "Admin", gr.update(selected="Workflow")
    elif username.startswith("dr_") and password == "pass":
        # This is for mock doctor users (e.g., dr_smith, dr_jones)
        return username, "Doctor", gr.update(selected="Workflow")
    else:
        # Login failed, show an error message.
        return None, None, gr.update(value="Login Failed. Try 'dr_smith'/'pass' or 'admin'/'123'.")
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    user_id_state = gr.State(value="") # e.g., dr_smith

    user_role_state = gr.State(value="") # e.g., Doctor


    # --- UI COMPONENTS ---

    # ✨ NEW: Use gr.Tabs to separate Login and Workflow
with gr.Tabs(selected="Login") as tabs:
    
    # 1. --- LOGIN TAB (Starts as the selected tab) ---
    with gr.TabItem("🔑 Login", id="Login"):
        gr.Markdown("## Login to the Cervical Cancer Diagnostic System")
        gr.Markdown("Please use a unique ID so your feedback can be tracked (e.g., 'dr_smith'/'pass' or 'admin'/'123').")
        
        login_username = gr.Textbox(label="User ID")
        login_password = gr.Textbox(label="Password", type="password")
        login_button = gr.Button("Login", variant="primary")
        login_status = gr.Textbox(label="Login Status", interactive=False)
        
        # Link the button to the authentication function
        login_button.click(
            fn=authenticate_user,
            inputs=[login_username, login_password],
            outputs=[user_id_state, user_role_state, tabs] # Update State and switch tab
        )
        # We use a second .then() to clear the password field after login attempt
        login_button.click(
    ...
    # 2. --- WORKFLOW TAB (Your entire existing app code goes inside here) ---
    with gr.TabItem("🔬 Clinical Workflow", id="Workflow"):
        
        # NOTE: Your existing code starts here (DEBUG OUTPUT and STAGE 1, 2, 3, 4 groups)
        
        # 👇 START INDENTATION HERE 👇
        
        debug_output = gr.Textbox(label="🐞 DEBUG OUTPUT")

        # --- UI COMPONENTS ---
        # The following content was in your original code and must be indented inside the TabItem:
    ...
    screener_grader_button.click(
        ...
        outputs=[
            screener_output_probs,      # Receives report on error, probs on success
            screener_ai_pred_state,
            diagnostic_image_state,
            screener_feedback_group,
            grader_feedback_group,
            rl_group,
            debug_output                # Receives report on error, "" on success
        ]
    )
